...
test_that("parAnnotation", {
  if ("annovarR" %in% .packages(all.available = T)) {
    ...
    expect_that(x[4, 2], equals("ID=COSM4732228;OCCURENCE=1(large_intestine)"))
    expect_that(x[5, 2], equals("ID=COSM383974,COSM5973818,COSM383973,COSM5973817;OCCURENCE=1(upper_aerodigestive_tract),1(lung)"))
    expect_that(is.na(x[6, 2]), equals(TRUE))
    row.cl <- makeCluster(2)
    x <- parAnnotation(dat = dat, anno.names = c("avsnp147", "cosmic81"), database.dir = database.dir, 
      db.type = "txt", row.cl = row.cl)
    stopCluster(row.cl)
    x <- as.data.frame(x)
    expect_that(colnames(x)[1], equals("avSNP147"))
    expect_that(colnames(x)[2], equals("COSMIC_81"))
