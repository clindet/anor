...
To install annovarR, firstly you need to install [R](https://cran.r-project.org/) interpreter (Supported Linux, MAC, and Windows). This package has been uploaded on The Comprehensive R Archive Network (CRAN, https://cran.r-project.org). You can use the command to install annovarR package easily:
...
Firstly, to use the annovarR annotation system, you need to get the annotation name through function `get.annotation.names()`. Then, you can use the function `get.download.name` to get the `download.name`. It can be used to download and install pointed annotation database. 

Besides, you can search the [vignette](https://life2cloud.com/tools/annovarR/articles/database_in_annovarR.html#overview-of-supported-database.html) to get the `annotation name` and `download name`.
...
Both R object and external file can be input in annovarR, and you need get the needed columns by `get.annotation.needcols()`. If only want to utilze the ANNOVAR supported database, you need prepare the followed format, refer the [tutorial](http://annovar.openbioinformatics.org/en/latest/user-guide/input/#annovar-input-file):
...
annovarR not only supported the text file but also supported the sqlite format database. We provide a simple function `sqlite.build` to build sqlite format database: input a text file and output a sqlite format database.
...
The `annotation` is the main function to access various annotation system including [ANNOVAR](http://annovar.openbioinformatics.org/en/latest/), [AnnotationDbi](http://www.bioconductor.org/packages/devel/bioc/html/AnnotationDbi.html), [vcfanno](https://github.com/brentp/vcfanno), [vep](http://asia.ensembl.org/info/docs/tools/vep/index.html), and built-in annotation system in annovarR.

If the `anno.name` contains `perl_annovar`, it will use the external `ANNOVAR` to finish the annotation step and read the output file as `data.table` object. 

Default annovarR use the R annotation system, mainly [data.table](https://github.com/Rdatatable/data.table) package. For the database with very large size, annovarR will convert the text file to `sqlite` format and generate the indexes.
...
If you want to use the ANNOVAR of perl version, you need to download the ANNOVAR source code using R package [BioInstaller](https://life2cloud.com/tools/bioinstaller/).
